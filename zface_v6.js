...
(function() {
	var loadPackage = function(metadata) {
		...
		var PACKAGE_NAME = "zface_v6.data";
		var REMOTE_PACKAGE_BASE = "zface_v6.data";
		...
		function runWithFS() {
			...
			Module["FS_createPath"]("/", "wasmModel", true, true);
			...
			function processPackageData(arrayBuffer) {
				...
				Module["removeRunDependency"]("datafile_zface_v6.data")
			}
			Module["addRunDependency"]("datafile_zface_v6.data");
	...
	loadPackage({
		"files": [{
			...
			"audio": 0,
			"end": 658150,
			"filename": "/wasmModel/toyger.face.dat"
		}],
		"remote_package_size": 658150,
		"package_uuid": "d4d1d79b-f2d5-43bb-93ad-c90a1a779207"
	})
})();
var moduleOverrides = {};
var key;
...
ENVIRONMENT_HAS_NODE = typeof process === "object" && typeof process.versions === "object" && typeof process.versions
	.node === "string";
	...
	switch (type) {
		...
		case "i64":
			tempI64 = [value >>> 0, (tempDouble = value, +Math_abs(tempDouble) >= +1 ? tempDouble > +0 ? (Math_min(+
				...
				(~~tempDouble >>> 0)) / +4294967296) >>> 0 : 0)], HEAP32[ptr >> 2] = tempI64[0], HEAP32[ptr + 4 >>
				2] = tempI64[1];
			break;
		case "float":
			HEAPF32[ptr >> 2] = value;
	...
	if (endPtr - idx > 16 && u8Array.subarray && UTF8Decoder) {
		...
		while (idx < endPtr) {
			...
			if ((u0 & 240) == 224) {
				...
				if ((u0 & 248) != 240) warnOnce("Invalid UTF-8 leading byte 0x" + u0.toString(16) +
...
function stringToUTF8Array(str, outU8Array, outIdx, maxBytesToWrite) {
	...
	for (var i = 0; i < str.length; ++i) {
		...
		if (u <= 127) {
			...
			if (u >= 2097152) warnOnce("Invalid Unicode code point 0x" + u.toString(16) +
...
var STACK_BASE = 5419776,
	STACK_MAX = 176896,
	DYNAMIC_BASE = 5419776,
	DYNAMICTOP_PTR = 176736;
...
assert(INITIAL_TOTAL_MEMORY >= TOTAL_STACK, "TOTAL_MEMORY should be larger than TOTAL_STACK, was " +
	INITIAL_TOTAL_MEMORY + "! (TOTAL_STACK=" + TOTAL_STACK + ")");
assert(typeof Int32Array !== "undefined" && typeof Float64Array !== "undefined" && Int32Array.prototype.subarray !==
	undefined && Int32Array.prototype.set !== undefined, "JS engine does not provide full typed array support");
...
function checkStackCookie() {
	...
	if (cookie1 != 34821223 || cookie2 != 2310721022) {
		abort("Stack overflow! Stack cookie has been overwritten, expected hex dwords 0x89BACDFE and 0x02135467, but received 0x" +
			cookie2.toString(16) + " " + cookie1.toString(16))
	}
	if (HEAP32[0] !== 1668509029) abort(
...
var wasmBinaryFile = "zface_v6.wasm";
...
var FS = {
	...
	rmdir: function(path) {
		...
		try {
			...
			console.log("FS.trackingDelegate['willDeletePath']('" + path + "') threw an exception: " + e
				.message)
		}
	...
	write: function(stream, buffer, offset, length, position, canOwn) {
		...
		try {
			...
			console.log("FS.trackingDelegate['onWriteToFile']('" + stream.path + "') threw an exception: " + e
				.message)
		}
		return bytesWritten
	},
...
var SYSCALLS = {
	...
	doStat: function(func, path, buf) {
		...
		HEAP32[buf + 32 >> 2] = 0;
		tempI64 = [stat.size >>> 0, (tempDouble = stat.size, +Math_abs(tempDouble) >= +1 ? tempDouble > +0 ? (
			Math_min(+Math_floor(tempDouble / +4294967296), +4294967295) | 0) >>> 0 : ~~+Math_ceil((
		...
		HEAP32[buf + 76 >> 2] = 0;
		tempI64 = [stat.ino >>> 0, (tempDouble = stat.ino, +Math_abs(tempDouble) >= +1 ? tempDouble > +0 ? (
			Math_min(+Math_floor(tempDouble / +4294967296), +4294967295) | 0) >>> 0 : ~~+Math_ceil((
			tempDouble - +(~~tempDouble >>> 0)) / +4294967296) >>> 0 : 0)], HEAP32[buf + 80 >> 2] = tempI64[
			0], HEAP32[buf + 84 >> 2] = tempI64[1];
		return 0
	},
...
function __embind_register_function(name, argCount, rawArgTypesAddr, signature, rawInvoker, fn) {
	...
	whenDependentTypesAreResolved([], argTypes, function(argTypes) {
		...
		replacePublicSymbol(name, craftInvokerFunction(name, invokerArgsArray, null, rawInvoker, fn), argCount -
			1);
		return []
	})
}
...
function _emscripten_get_now_is_monotonic() {
	return 0 || ENVIRONMENT_IS_NODE || typeof dateNow !== "undefined" || typeof performance === "object" &&
...
function emscripten_realloc_buffer(size) {
	try {
		...
		console.error("emscripten_realloc_buffer: Attempted to grow heap from " + buffer.byteLength + " bytes to " +
...
function _emscripten_resize_heap(requestedSize) {
	...
	if (requestedSize > LIMIT) {
		err("Cannot enlarge memory, asked to go up to " + requestedSize + " bytes, but the limit is " + LIMIT +
			" bytes!");
		return false
	}
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
	try {
		...
		tempI64 = [stream.position >>> 0, (tempDouble = stream.position, +Math_abs(tempDouble) >= +1 ? tempDouble > +0 ?
				(Math_min(+Math_floor(tempDouble / +4294967296), +4294967295) | 0) >>> 0 : ~~+Math_ceil((
...
function _strftime(s, maxsize, format, tm) {
	...
	var EXPANSION_RULES_2 = {
		...
		"%W": function(date) {
			...
			var firstMonday = janFirst.getDay() === 1 ? janFirst : __addDays(janFirst, janFirst.getDay() === 0 ?
				1 : 7 - janFirst.getDay() + 1);
			var endDate = new Date(date.tm_year + 1900, date.tm_mon, date.tm_mday);
...
function exit(status, implicit) {
	...
	if (noExitRuntime) {
		if (!implicit) {
			err("program exited (with status: " + status +
